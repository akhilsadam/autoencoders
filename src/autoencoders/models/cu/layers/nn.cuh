...
struct module {
    ...
    __device__ __forceinline__ uint64_t eval(typename shared_allocator<int>& al, const int32_t _in_chan, const uint64_t x_ptr) {
        in_chan = _in_chan;
    ...
    __device__ __forceinline__ uint64_t train(typename shared_allocator<int>& al, const uint64_t grad_y_ptr) {
        // allocate
        if (grad_y_ptr != 0) 
        {
    ...
    virtual __device__ __forceinline__ void init_weights(typename shared_allocator<int>& al) {
...
struct module_chain {
    ...
    __device__ inline uint64_t eval(typename shared_allocator<int>& al, const uint32_t in_chan, const uint64_t x_ptr = 0) {
    ...
    __device__ inline uint64_t train(typename shared_allocator<int>& al, const uint64_t grad_y_ptr = 0) {
        auto* curr_grad_y_ptr = next.train(al, grad_y_ptr);
        return current.train(al, curr_grad_y_ptr);
    }

    // iterate through to init weights
    __device__ inline void init_weights(typename shared_allocator<int>& al) {
