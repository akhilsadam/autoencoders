    ...
    for(int i = 0; i < A.height; i++) {
        ...
        for(int j = 0; j < A.width; j++) {
            ...
            for(int k = 0; k < A.packed_per_tile; k++) {
                // sine(x)
                A.tiles[i][j].data[k].x = sinf(X.tiles[i][j].data[k].x);
                A.tiles[i][j].data[k].y = sinf(X.tiles[i][j].data[k].y);
            }
        }
    }
}
...
__device__ static inline void act_sine_bwd(T &A, const T &X, const T &GY) {
    ...
    for(int i = 0; i < A.height; i++) {
        ...
        for(int j = 0; j < A.width; j++) {
            ...
            for(int k = 0; k < A.packed_per_tile; k++) {
                // sine(x)
                A.tiles[i][j].data[k].x = cosf(X.tiles[i][j].data[k].x) * GY.tiles[i][j].data[k].x;
                A.tiles[i][j].data[k].y = cosf(X.tiles[i][j].data[k].y) * GY.tiles[i][j].data[k].y;
            }
        }
    }
}
