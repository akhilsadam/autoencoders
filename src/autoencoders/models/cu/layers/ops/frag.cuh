...
template<ducks::rt::all T> // T2, w, h can be inferred from dst as long as op is specialized
__device__ static inline void scalar_fma_map(T &A, const T &B, ftype &w, const T &D) {
    #pragma unroll
    for(int i = 0; i < A.height; i++) {
        #pragma unroll
        for(int j = 0; j < A.width; j++) {
            #pragma unroll
            for(int k = 0; k < A.packed_per_tile; k++) {
                A.tiles[i][j].data[k].x = B.tiles[i][j].data[k].x * w + D.tiles[i][j].data[k].x;
                A.tiles[i][j].data[k].y = B.tiles[i][j].data[k].y * w + D.tiles[i][j].data[k].y;
            }
        }
    }
}
...
template<int32_t c_in, int32_t k_in, ducks::rt::all T, ducks::rt::all U>
__device__ static inline void cast_flat_to_tile(T (&A)[c_in], const U &A_flat) {
    ...
    for (int c = 0; c < c_in; ++c) {
        ...
        for(int j = 0; j < A[0].height; j++) {
            ...
            for(int i = 0; i < A[0].width; i++) {
                ...
                #pragma unroll
                for(int k = 0; k < A[0].packed_per_tile; k++) {
                    A[c].tiles[j][i].data[k].x = __bfloat162float(A_flat.tiles[n][l].data[k].x);
                    A[c].tiles[j][i].data[k].y = __bfloat162float(A_flat.tiles[n][l].data[k].y);
                }
            }
        }
    }
