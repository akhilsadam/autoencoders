...
void run_relu_fwd_kernel(torch::Tensor x, torch::Tensor y) {
}
void run_relu_bwd_kernel(torch::Tensor grad_y, torch::Tensor y, torch::Tensor grad_x) {
}
...
PYBIND11_MODULE(act, m) {
    ...
    m.def("relu_fwd", &run_relu_fwd_kernel);
    m.def("relu_bwd", &run_relu_bwd_kernel);
