...
def _check(true_func, cu_func):
    for size in sizes:
        ...
        try:
            y = true_func(x)
            y_hat = cu_func(x_2)
            assert torch.allclose(y_hat, y, atol=0.008), f"Forward check failed for size {size}"
            ...
            x_g_hat = x_2.grad
            assert torch.allclose(x_g, x_g_hat, atol=0.008), f"Gradient check failed for size {size}"
