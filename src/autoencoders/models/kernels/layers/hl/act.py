...
def _sine_bwd(g: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    return g * torch.cos(y)

def _silu_fwd(x: torch.Tensor) -> torch.Tensor:
    return x * torch.sigmoid(x)

def _silu_bwd(g: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    sig = torch.sigmoid(y)
    return g * (sig * (1 + y * (1 - sig)))
