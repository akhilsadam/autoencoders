...
from autoencoders.metrics import image_diffusion as MX
...
class Diffusion(pl.LightningModule):
    ...
    def loss(self, x: torch.Tensor) -> torch.Tensor:
        ...
        n      = self.noise(x)
        x_n    = self.mix(x, n, t)
        v_pred = self.vel(self.denoise(x_n, t), x_n, t) * (1 - t)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        n = self.noise(x)
        return self.denoise(self.mix(x, n, t), t)

    def latent(self, x: torch.Tensor) -> torch.Tensor:
        return self.noise(x)

    def gen(self, x: torch.Tensor, shape=None, L=None) -> torch.Tensor:
        ...
        for i in range(self.steps):
            x_n = self.sampler.step(self, x_n, i, self.t, self.dt)
        return x_n
    ...
    def validation_step(self, batch: torch.Tensor, _: int) -> None:
        self.log('val_loss', self.loss(batch[0]), prog_bar=True)
